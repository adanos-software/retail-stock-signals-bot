Title: Daily Retail Stock Signals - May 15, 2026: NVDA Leads Buzz, SHOP Moves Most

# Daily Retail Stock Signals - May 15, 2026

Today's split: **NVDA** led raw buzz, but **TTM** had the cleaner sentiment profile.

### Signal Summary

**Top Buzz: NVDA**  
Buzz **82.0** | Sentiment **positive** | Bull/Bear **30% / 20%**  
The broadest attention signal in today's Reddit data.

**Best Sentiment Read: TTM**  
7D Buzz **+14.9** (52.0 -> 66.9) | Sentiment **strong positive** (+0.225) | Bull/Bear **54% / 10%**  
The best-confirmed sentiment read: buzz rose and bullish discussion stayed clearly ahead of bearish discussion.

**Largest 7-Day Buzz Move: SHOP**  
7D Buzz **+39.5** (26.6 -> 66.1) | Sentiment **neutral** (-0.018) | Bull/Bear **24% / 27%**  
The largest attention jump, but sentiment does not strongly confirm it.

**7-Day Buzz Fade: SNDK**  
7D Buzz **-3.4** (78.3 -> 74.9) | Sentiment **neutral** (+0.025) | Bull/Bear **30% / 23%**  
Attention cooled over 7 days, but the sentiment read is not strongly negative.

### Top Buzz

**1. NVDA** - Buzz **82.0**, sentiment **positive**, rising  
**2. MSFT** - Buzz **80.8**, sentiment **positive**, rising  
**3. AMZN** - Buzz **78.2**, sentiment **strong positive**, stable  
**4. AAPL** - Buzz **78.0**, sentiment **neutral**, rising  
**5. INTC** - Buzz **77.9**, sentiment **neutral**, falling  

### 7-Day Movers

**SHOP** - **+39.5** buzz, buzz up, sentiment neutral  
**BA** - **+18.9** buzz, buzz up, sentiment neutral  
**GM** - **+15.3** buzz, buzz up, sentiment neutral  
**TTM** - **+14.9** buzz, strong positive sentiment  
**BABA** - **+11.5** buzz, positive momentum  
**SNDK** - **-3.4** buzz, buzz fading  
**NOK** - **-3.2** buzz, buzz fading  
**INTC** - **-2.9** buzz, buzz fading  

### Takeaway

NVDA leads absolute buzz, but sentiment is only mildly positive. TTM shows the cleanest bullish tilt with a strong positive score and high bull/bear ratio. SHOP's buzz surged sharply but sentiment remains neutral, suggesting volume without conviction.

Would you trust **NVDA's broad attention** or **TTM's cleaner sentiment read** more here?

Data: Adanos Reddit stock sentiment - https://adanos.org/reddit-stock-sentiment  
Data-driven sentiment signal, not financial advice.
